...
let AgentHostSessionTitleController = class extends Disposable {
  ...
  async _generateTitleFromPrompt(promptContent, isConversation, token) {
    ...
    try {
      const titlePromptContent = isConversation ? promptContent : await this._appendGitHubContext(promptContent, abortController.signal, token);
      if (token.isCancellationRequested) {
        return void 0;
      }
  ...
  async _appendGitHubContext(promptContent, cancellationSignal, token) {
...
${this._formatGitHubContexts(successfulContexts)}`;
  ...
  _formatGitHubContexts(contexts) {
