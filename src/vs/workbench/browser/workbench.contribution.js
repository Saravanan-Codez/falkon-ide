...
Registry.as(Extensions.ConfigurationMigration).registerConfigurationMigrations([{
  ...
  migrateFn: (value, valueAccessor) => {
    const result = [["workbench.experimental.floatingPanels", { value: void 0 }]];
    if (value === true && valueAccessor(LayoutSettings.MODERN_UI) === void 0) {
  ...
  migrateFn: (value, valueAccessor) => {
    const result = [["workbench.experimental.styleOverrides", { value: void 0 }]];
    if (Array.isArray(value) && value.length > 0 && valueAccessor(LayoutSettings.MODERN_UI) === void 0) {
      result.push([LayoutSettings.MODERN_UI, { value: true }]);
    }
    return result;
  }
