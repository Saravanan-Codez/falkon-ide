...
const ROUTE_AUTOSEND_DELAY_MS = 1e4;
...
let ChatSessionRoutingController = class extends Disposable {
  constructor(host, debugOwner, chatService, agentSessionsService, chatSessionsService, sessionRouter, chatWidgetService, logService, workspaceContextService, newSessionFolderService, quickInputService) {
    super();
    this.host = host;
  ...
  async handleSubmit(query, _mode, attachedContext, isVoiceModeInput) {
    ...
    if (explicitNewSessionTask) {
      ...
      const target2 = this._resolveNewSessionTarget(utterance, attachedContext, [], []);
      this._dispatchImmediately(target2, query, submittedAttachmentIds, utterance, requestOptions, cts);
      return true;
    }
  ...
  _resolveNewSessionTarget(utterance, attachedContext, results, candidates) {
    ...
    const folder = this._folderFromAttachments(attachedContext) ?? resolveNewSessionWorkspaceFolder(utterance, folders, results, candidates, this.newSessionFolderService.getDefaultFolder());
    return {
      kind: "new",
      label: folder ? localize("chatSessionRouting.newSessionInFolder", "New session in {0}", this.workspaceContextService.getWorkspaceFolder(folder)?.name ?? folder.path) : localize("chatSessionRouting.newSession", "New session"),
      folder
    };
  }
  _folderFromAttachments(attachedContext) {
  ...
  _renderCountdownBadge(badge, store, target, newSessionTarget, results, candidates, submittedInput, submittedAttachmentIds, utterance, requestOptions, cts) {
    ...
    const rows = options.map((option, index) => {
      ...
      score.textContent = option.kind === "session" ? index === 0 ? localize("chatSessionRouting.bestMatchSessionModel", "Best Match \xB7 Session model") : localize("chatSessionRouting.highConfidenceSessionModel", "High Confidence \xB7 Session model") : requestOptions.userSelectedModelId ? localize("chatSessionRouting.selectedModel", "Selected model") : "";
    ...
    ariaAlert(initialTarget.kind === "session" ? localize("chatSessionRouting.sendingToIn", "Sending to {0} in {1} seconds. Press Escape to cancel.", initialTarget.label, Math.ceil(ROUTE_AUTOSEND_DELAY_MS / 1e3)) : localize("chatSessionRouting.confirmNewSession", "No confident match. Choose a destination before sending."));
    let remainingSeconds = Math.ceil(ROUTE_AUTOSEND_DELAY_MS / 1e3);
    ...
    const send = () => {
      ...
      if (sent.length > 1) {
        this.host.onDidResolveRoute?.(void 0);
      }
    ...
    const startCountdown = () => {
      remainingSeconds = Math.ceil(ROUTE_AUTOSEND_DELAY_MS / 1e3);
      renderCountdown();
  ...
  _trackDeliveryActivity(store, resource, label, mark, labelElement, waitForActivity) {
  ...
  _showDispatchFailure(label) {
    const badge = dom.$(".chat-routing-badge");
    const mark = dom.append(badge, dom.$("span.chat-routing-badge-sent-mark"));
    mark.appendChild(renderIcon(Codicon.error));
    const message = dom.append(badge, dom.$("span.chat-routing-badge-label"));
    message.textContent = label ? localize("chatSessionRouting.sendFailedTo", "Could not send to {0}. Your draft was preserved.", label) : localize("chatSessionRouting.sendFailed", "Could not send the request. Your draft was preserved.");
    this.host.placeBadge(badge);
    if (!badge.parentElement) {
      return;
    }
  ...
  async _dispatchToNewSession(submittedInput, submittedAttachmentIds, utterance, requestOptions, token, notifyRoute, folder) {
    let routeResource;
    try {
