...
class ServerEnvironmentService extends NativeEnvironmentService {
  ...
  get reconnectionGraceTime() {
    return parseGraceTime(this.args["reconnection-grace-time"], ProtocolConstants.ReconnectionGraceTime);
  }
}
__decorateClass([
  memoize
...
__decorateClass([
  memoize
], ServerEnvironmentService.prototype, "machineSettingsResource", 1);
__decorateClass([
  memoize
], ServerEnvironmentService.prototype, "mcpResource", 1);
__decorateClass([
  memoize
], ServerEnvironmentService.prototype, "reconnectionGraceTime", 1);
function parseGraceTime(rawValue, fallback) {
  if (typeof rawValue !== "string" || rawValue.trim().length === 0) {
    console.log(`[reconnection-grace-time] No CLI argument provided, using default: ${fallback}ms (${Math.floor(fallback / 1e3)}s)`);
    return fallback;
  }
  const parsedSeconds = Number(rawValue);
  if (!isFinite(parsedSeconds) || parsedSeconds < 0) {
    console.log(`[reconnection-grace-time] Invalid value '${rawValue}', using default: ${fallback}ms (${Math.floor(fallback / 1e3)}s)`);
    return fallback;
  }
  const millis = Math.floor(parsedSeconds * 1e3);
  if (!isFinite(millis) || millis > Number.MAX_SAFE_INTEGER) {
    console.log(`[reconnection-grace-time] Value too large '${rawValue}', using default: ${fallback}ms (${Math.floor(fallback / 1e3)}s)`);
    return fallback;
  }
