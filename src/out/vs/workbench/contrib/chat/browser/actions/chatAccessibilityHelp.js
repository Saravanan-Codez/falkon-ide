...
class ChatInputWindowAccessibilityHelp {
  constructor() {
    this.priority = 121;
    this.name = "chatInputWindow";
    this.type = AccessibleViewType.Help;
    this.when = ChatContextKeys.inChatInputWindow;
  }
  getProvider(accessor) {
    return getChatAccessibilityHelpProvider(accessor, void 0, "chatInputWindow");
  }
}
...
function getAccessibilityHelpText(type, keybindingService, supportsFileReferences, isSessionsWindow = false, stickyPromptHeaderShown = false, inputWindowEnabled = false) {
  const content = [];
...
function getChatAccessibilityHelpProvider(accessor, editor, type) {
  ...
  const helpText = getAccessibilityHelpText(
    ...
    configurationService.getValue(OmniChatEnabledSettingId)
  );
...
export {
  ...
  ChatInputWindowAccessibilityHelp,
